...
import { type ImageSource, createImageMediaData } from "@file/media/image-data";
...
import { createExtentionList } from "../../pic/blip/blip-extentions";
import { type ICropOptions, SourceRectangle } from "../../pic/blip/source-rectangle";
import { Stretch } from "../../pic/blip/stretch";
import { createNoFill } from "../../pic/shape-properties/outline/no-fill";

export type { ImageSource } from "@file/media/image-data";
    ...
    new BuilderElement<{ readonly embed: string }>({
        ...
        children: [
            ...(transparency
                ...
                : []),
            ...(mediaData.type === "svg" ? [createExtentionList(mediaData)] : []),
        ],
    });
...
class PictureFill extends XmlComponent {
    ...
    public constructor({ image, tile, crop, transparency }: Omit<PictureShapeFill, "type">, name: string) {
        ...
        this.root.push(new SourceRectangle(tile ? undefined : crop));
        this.root.push(tile ? createTile(tile) : new Stretch());
    }
