 ...
 *
 * For each preset shape it keeps what is needed to find the shape's connection sites:
 * the default adjustment values (`a:avLst`), the guides the sites depend on (`a:gdLst`) and the sites (`a:cxnLst`).
 *
 * Usage:
...
const main = async (): Promise<void> => {
    ...
    for (const [name, ooxmlName] of Object.entries(PRESET_SHAPE_OOXML_NAMES)) {
        ...
        // Keep only the guides the sites depend on, in their original order. Some shapes define a guide
        // more than once, so every definition of a guide that is needed is kept
        const allGuides = guides(shape, "gdLst");
        const needed = new Set<string>();
        const pending = sites.flat();
